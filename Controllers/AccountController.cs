...
namespace GameCraft.Controllers
{
    public class AccountController : Controller
    {
        ...
        public AccountController(GameCraftDbContext context, IWebHostEnvironment env, EmailSender emailSender)
        {
            _context = context;
            _env = env;
        ...
        public async Task<IActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                AccountController.TokenStore[token] = model.Email;

                string callbackUrl = $"https://localhost:44338/Account/VerifyEmail?token={token}";
                await _emailSender.SendVerificationEmail(model.Email, callbackUrl);
        ...
        public async Task<IActionResult> SendVerificationEmail(string email)
        {
            if (string.IsNullOrWhiteSpace(email))
                return BadRequest("The email field cannot be left blank.");

            string token = Guid.NewGuid().ToString();

            // Save the Token, demonstrate using the database or TempData
            TempData["VerificationToken"] = token;
            TempData["VerificationEmail"] = email;

            await _emailSender.SendVerificationEmail(email, token);

            ViewBag.Message = "The verification email has been sent. Please check your inbox.";
